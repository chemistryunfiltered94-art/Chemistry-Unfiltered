// app/(public)/learn/[category]/[chapterId]/[topicSlug]/page.tsx
// টপিকে ক্লিক করলে সেই টপিকের সাব-টপিকগুলো (আসল কনটেন্ট পেজের তালিকা) দেখাবে

import { Metadata } from "next";
import Link from "next/link";
import { notFound } from "next/navigation";
import { ArrowLeft, Clock, ArrowRight, BookOpen } from "lucide-react";
import { getContentChapter, findTopicBySlug } from "@/lib/seedContent";
...
interface Props { params: Promise<{ category: string; chapterId: string; topicSlug: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { chapterId, topicSlug } = await params;
  const found = findTopicBySlug(chapterId, topicSlug);
  if (!found) return { title: "Not Found" };
  return { title: `${found.topic.title} — সাব-টপিকসমূহ` };
}

export default async function TopicSubtopicsPage({ params }: Props) {
  const { category, chapterId, topicSlug } = await params;
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-4 py-6">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Back */}
        <Link
          href={`/learn/${category}/${chapterId}`}
          className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-5 transition-colors text-sm"
        >
          <ArrowLeft className="w-4 h-4" /> {chapter.title}
        </Link>

        {/* Topic header */}
        <div className={`bg-gradient-to-r ${catMeta.color} rounded-2xl p-5 text-white mb-6`}>
          <p className="text-white/70 text-sm mb-1">{catMeta.name} • {chapter.title}</p>
          <h1 className="text-2xl font-bold">{topic.title}</h1>
          {topic.summary && (
            <p className="text-white/70 text-sm mt-1">{topic.summary}</p>
          )}
          <p className="text-white/60 text-xs mt-2">{subtopics.length}টি সাব-টপিক</p>
        </div>
        ...
        {subtopics.length === 0 ? (
          <div className="text-center py-14 text-slate-500">
            ...
            <p className="text-sm">এই টপিকে এখনো কোনো সাব-টপিক যোগ করা হয়নি।</p>
          </div>
        ) : (
          <div className="space-y-3">
            {subtopics.map((sub, i) => (
              <Link
                key={sub.slug}
                href={`/learn/${category}/subtopic/${sub.slug}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {sub.summary && (
                    <p className="text-xs text-slate-400 mt-0.5 truncate">{sub.summary}</p>
                  )}
                  <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                    <Clock className="w-3 h-3" />
                    {sub.estimatedTime} মিনিট
                  </p>
                </div>
