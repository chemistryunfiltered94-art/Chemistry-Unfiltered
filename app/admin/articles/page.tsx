...
import { ArrowLeft, FileText, Search, Plus, Trash2, Eye, EyeOff } from "lucide-react";

export default function AdminArticlesPage() {
  const { isAdmin } = useAuth();
  const { data: articles, loading } = useCollection<Article>("articles");
  ...
  const togglePublished = async (article: Article) => {
    ...
    const ok = await updateDocument("articles", article.id, { published: !article.published });
    setBusyId(null);
  ...
  const handleDelete = async (id: string) => {
    setBusyId(id);
    const ok = await deleteDocument("articles", id);
    setBusyId(null);
    setConfirmDeleteId(null);
    if (ok) showToast("success", "আর্টিকেল মুছে ফেলা হয়েছে।");
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">আর্টিকেল ম্যানেজ</h1>
              <p className="text-slate-400 text-sm">মোট {articles.length}টি আর্টিকেল</p>
            </div>
          </div>
          <Link href="/admin/articles/new"
            ...
            <Plus className="w-4 h-4" /> নতুন আর্টিকেল
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl text-center py-16 text-slate-400">
            ...
            <Link href="/admin/articles/new" className="inline-block mt-3 text-primary-400 hover:text-primary-300 text-sm font-medium">
              প্রথম আর্টিকেল লেখো →
            </Link>
          </div>
        ) : (
            ...
            {filtered.map((article, i) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 truncate">/{article.slug}</p>
                </div>

                <button onClick={() => togglePublished(article)} disabled={busyId === article.id}
                  ...
                  } ${busyId === article.id ? "opacity-50" : "hover:opacity-80"}`}
                >
                  {article.published ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                  {article.published ? "প্রকাশিত" : "অপ্রকাশিত"}
                </button>

                {confirmDeleteId === article.id ? (
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => handleDelete(article.id)} disabled={busyId === article.id}
                  ...
                  <button onClick={() => setConfirmDeleteId(article.id)}
