...
import { ArrowLeft, Atom, Search, Plus, Trash2 } from "lucide-react";

export default function AdminFormulasPage() {
  const { isAdmin } = useAuth();
  const { data: formulas, loading } = useCollection<Formula>("formulas");
  ...
  const handleDelete = async (id: string) => {
    setBusyId(id);
    const ok = await deleteDocument("formulas", id);
    setBusyId(null);
    setConfirmDeleteId(null);
    if (ok) showToast("success", "ফর্মুলা মুছে ফেলা হয়েছে।");
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">ফর্মুলা ম্যানেজ</h1>
              <p className="text-slate-400 text-sm">মোট {formulas.length}টি ফর্মুলা</p>
            </div>
          </div>
          <Link href="/admin/formulas/new"
            ...
            <Plus className="w-4 h-4" /> নতুন ফর্মুলা
          </Link>
        </div>
        ...
        <div className="relative mb-5">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="ফর্মুলার নাম দিয়ে খোঁজো..."
        ...
        {loading ? (
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl text-center py-16 text-slate-400">
            ...
            <Link href="/admin/formulas/new" className="inline-block mt-3 text-primary-400 hover:text-primary-300 text-sm font-medium">
              প্রথম ফর্মুলা যোগ করো →
            </Link>
          </div>
        ) : (
            ...
            {filtered.map((f, i) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-white truncate">{f.nameBn || f.name}</p>
                  <p className="text-xs text-slate-500 mt-0.5 font-mono truncate">{f.formula}</p>
                </div>

                {confirmDeleteId === f.id ? (
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => handleDelete(f.id)} disabled={busyId === f.id}
